...
const MenuButton = React.forwardRef<HTMLButtonElement, MenuButtonProps>(({ href, icon, children, letter, className, variant = 'default', size = 'default', ...props }, ref) => {
  const ButtonContent = (
    <Button ref={ref} variant={variant} size={size} className={cn('flex !gap-2 flex-row justify-between items-center group', className)} {...props}>
